...
func (c *SPDNSProviderSolver) getZoneID(stackpath *spdns.APIClient, stack string, auth context.Context, zone string) string {
	request := stackpath.ZonesApi.GetZones(auth, stack)
	request = request.PageRequestFilter(fmt.Sprintf("domain = \"%s\"", zone))
	zr, resp, err := request.Execute()
	if err != nil {
	...
	zones := zr.GetZones()
	if len(zones) == 0 {
		fmt.Fprintf(os.Stderr, "No zone matching %v in stack %v\n", zone, stack)
		return ""
	}
...
func (c *SPDNSProviderSolver) Present(ch *v1alpha1.ChallengeRequest) error {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "failed to get client from ChallengeRequest: %v\n", err)
		return err
	}

	request := stackpath.ResourceRecordsApi.CreateZoneRecord(
		auth,
		stack,
		c.getZoneID(stackpath, stack, auth, strings.TrimSuffix(ch.ResolvedZone, ".")),
	)
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "Error when calling `ResourceRecordsApi.CreateZoneRecord` for %s: %v\n", recordName, err)
		fmt.Fprintf(os.Stderr, "Full HTTP response: %v\n", r)
		return err
	}
...
func (c *SPDNSProviderSolver) CleanUp(ch *v1alpha1.ChallengeRequest) error {
	...
	zoneId := c.getZoneID(stackpath, stack, auth, strings.TrimSuffix(ch.ResolvedZone, "."))

	request := stackpath.ResourceRecordsApi.GetZoneRecords(auth, stack, zoneId)
	request = request.PageRequestFilter(fmt.Sprintf("data = \"%s\"", ch.Key))
	rr, resp, err := request.Execute()
	if err != nil {
	...
	if len(records) == 0 {
		fmt.Fprintf(os.Stderr, "No record matching with value %v for domain %v in stack %v\n", ch.Key, strings.TrimSuffix(ch.ResolvedZone, "."), stack)
		return err
	}

	resp, err = stackpath.ResourceRecordsApi.DeleteZoneRecord(
		auth,
		stack,
		zoneId,
		records[0].GetId(),
